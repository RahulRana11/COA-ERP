...
export default function ApprovedRenewalsList() {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Approved Renewals</h1>
                <p className="text-muted-foreground">Successfully processed and granted renewal applications.</p>
            </div>
                            ...
                            requests.map((req) => (
                                <TableRow key={req.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Link to={`/official/review/${req.id}`} className="text-blue-600 hover:underline">View Receipt</Link>
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No approved renewals yet.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
