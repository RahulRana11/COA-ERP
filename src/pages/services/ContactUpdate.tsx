...
export default function ContactUpdate() {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            const { data, error } = await supabase
                ...
                `)
                .eq('request_type', 'Contact Update');
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Email / Mobile Update</h1>
                <p className="text-muted-foreground">Manage requests for primary contact modification, usually involving OTP verification.</p>
            </div>
                            ...
                            requests.map((req) => (
                                <TableRow key={req.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Link to={`/official/review/${req.id}`} className="text-blue-600 hover:underline">Review Logs</Link>
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No contact update requests found.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
