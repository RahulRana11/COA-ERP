...
export default function PhotoSignatureChange() {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            const { data, error } = await supabase
                ...
                `)
                .in('request_type', ['Photo Update', 'Signature Update', 'Photo / Signature Update']);
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Photo & Signature Change</h1>
                <p className="text-muted-foreground">Manage profile image and digital signature update requests.</p>
            </div>
                            ...
                            requests.map((req) => (
                                <TableRow key={req.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Link to={`/official/review/${req.id}`} className="text-blue-600 hover:underline">Compare Media</Link>
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No media change requests found.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
