...
export default function NameChangeRequests() {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Name Change Requests</h1>
                <p className="text-muted-foreground">Review applications for legal name changes, requiring gazette notification or marriage certificate.</p>
            </div>
                ...
                <Table>
                    <TableHeader className="bg-muted/50">
                        <TableRow>
                            ...
                            <TableHead>Current Registered Name</TableHead>
                            ...
                            requests.map((req) => (
                                <TableRow key={req.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Link to={`/official/review/${req.id}`} className="text-blue-600 hover:underline">Review Document</Link>
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No name change requests found.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
