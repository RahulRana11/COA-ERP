...
export default function AddressChange() {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            const { data, error } = await supabase
                ...
                `)
                .eq('request_type', 'Address Update');
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Address Change Requests</h1>
                <p className="text-muted-foreground">Review applications for updating residential or professional addresses.</p>
            </div>
                            ...
                            requests.map((req) => (
                                <TableRow key={req.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <Link to={`/official/review/${req.id}`} className="text-blue-600 hover:underline">Verify Proof</Link>
                                    </TableCell>
                                </TableRow>
                            ))
                        ) : (
                            <TableRow>
                                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No address change requests found.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
